...
async function calculateTotalPrice() {
    ...
    let increase2 = 0;
    if (option2.checked) {
        increase2 = totalPrice * 0.25;
    }

    let option3 = document.querySelector('#option-presents2');
    let increase3 = 0;
    if (option3.checked) {
        increase3 = totalPrice * 0.3;
    }
    ...
    if (increase1 != 0 || increase2 != 0 || increase3 != 0) {
        totalPrice = Math.round(totalPrice - increase1 + increase2 + increase3);
        spanTotalPrice.innerHTML = totalPrice + ' руб.';
    }
...
function optionHandler(event) {
    let option = event.target.querySelector('#option-presents2');
    calculateTotalPrice();
}
...
function downloadGuides(route_id) {
    ...
    url.search = 'api_key=26f074f0-90b0-416a-87bc-47fb054836ec';
    let xhr = new XMLHttpRequest();
    xhr.open('GET', url);
    xhr.responseType = 'json';
    xhr.onload = function () {
